...
def main(reference, query, output_file):
    ...
    cwd = os.getcwd()
    os.chdir(ref_path[0])
    chroma_reference2 = np.genfromtxt(ref_path[1], delimiter=',')  
      
    os.chdir(cwd)
    os.chdir(query_path[0])
    chroma_query2 = np.genfromtxt(query_path[1], delimiter=',')

    chroma_reference = chroma_query2 # for reversal
    chroma_query = chroma_reference2 # for reversal
