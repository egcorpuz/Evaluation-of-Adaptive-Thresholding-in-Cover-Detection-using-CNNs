...
const (
	inputDir  = "./3_eval_adaptive2" // with 031 folder
	OutDir    = "5_eval_noncovers_img_adaptive"
	songLimit = 465 //total folder to folder iterations (nCr = 31C2)
	fileLimit = 222465 // total pairs
	procCount = 20
)
...
func crossSimilarity(stop chan bool, c chan [2]*exec.Cmd, refDir string, queryDir string) {
	...
	for _, refFile := range refFiles {
		for _, queryFile := range queryFiles {
			...
			jpgOutputDir := fmt.Sprintf("%v-r.jpg", strings.TrimRight(outputDir, ".csv")) // %v-r if reversed
			...
			csvCommand := exec.Command("./crosssimilarity2.py", refFileDir, queryFileDir, outputDir) // crosssimilarity2.py if reversed
